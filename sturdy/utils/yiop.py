...
def robust_target_function(x, pools):
    pool_items = [pool for idx, pool in pools.items()]
    total_yield = 0
    for idx, pool in enumerate(pool_items):
        for r in np.arange(0.8, 1.01, 0.05):
            alloc = x[idx]
            util_rate = pool["borrow_amount"] * r / (pool["reserve_size"] + alloc)
            pool_yield = alloc * supply_rate(util_rate, pool)
            total_yield += pool_yield
    return -total_yield


def robust_yiop_allocation_algorithm(synapse: sturdy.protocol.AllocateAssets) -> Dict:
    ...
    res = minimize(robust_target_function, x0, args=(pools), method='SLSQP', constraints=cons, bounds=bnds)
